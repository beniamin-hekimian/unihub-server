...
async function createProfessor(req, res) {
  try {
    const { name, email, password, role, gender, phone, department } = req.body;
    ...
    const populatedProfessor = await Professor.findById(professor._id).populate(
      "userId"
    );
...
async function updateProfessor(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to update professor", error: error.message });
  }
}
...
async function deleteProfessor(req, res) {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to delete professor", error: error.message });
  }
}
